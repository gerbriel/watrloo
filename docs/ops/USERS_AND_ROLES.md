...
| **moderator** | Granted by an admin (insert a `user_roles` row) — but the role alone is **inert**: power comes from assignments (bathrooms directly, or orgs whose verified claims cover bathrooms), granted by an admin. | Within their assigned scope ONLY (migration `20260714010000`, `moderates_bathroom()`): update a bathroom, soft-delete/restore a bathroom or review, permanently delete a single review photo (explicit content — destroys the bytes, audited, no restore), see removed content, read + resolve `reports` on in-scope targets. Outside scope, RLS and every RPC refuse them. | Scoping caps the blast radius of any one moderator account: a compromised or rogue moderator can touch only their beat. A moderator with zero assignments can moderate nothing. Volume role — granted somewhat liberally, so it deliberately **cannot** mint more roles. |
